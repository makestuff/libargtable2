...
ifeq ($(OS),Windows_NT)
	...
	GET_ARGTABLE = PATCH=paatch ./paatch.sh
else
	...
	GET_ARGTABLE = PATCH=patch ./paatch.sh
endif
PRE_BUILD := foo $(CC_SRCS)
EXTRA_CLEAN := *.c *.h *.def argtable2-12
CONFIGS := 
...
argtable2.tgz:
	wget -O argtable2.tgz http://downloads.sourceforge.net/project/argtable/argtable/argtable-2.12/argtable2-12.tar.gz